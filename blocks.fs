...
: flush 32 >r begin r@ 1- write next ; hide .2
...
: ASM 4 23 thru ; \ load in assembler
...
\ ASM: MEMORY DEREFERENCES + DISPLACEMENTS, MISCELLANEOUS
...
: a? >r/m 0= ; : cl? dup >type type-r8 = swap >r/m 1 = and ;
: short? dest >size 0= ;
\ ASM: ARGUMENT PARSING
...
\ ASM: COMPILING ASSEMBLY WORDS
0 value op : >op to op ; 0 value /n : >/n to /n ;
0 value ex : >ex to ex ;
: modr/m, ( mod reg r/m -- ) modr/m c, ;
: ds,  ( mod -- ) d >r/m s >r/m modr/m, ;
: sd,  ( mod -- ) s >r/m d >r/m modr/m, ;
: d, ( mod n -- ) d >r/m modr/m, ; : nd,  ( mod -- ) /n d, ;
: ^ s d L>M or to args ;
: reg, ( op -- ) c, mod-reg sd,    ;
: mem, ( op -- ) c, mod-mem sd,    ;
: d8,  ( op -- ) c, mod-d8  sd, c, ;
...
\ ASM: COMPILING ASSEMBLY WORDS, MOV
...
: acc, ( disp? -- )
  ...
  !abort ; : INC 0 >/n $40 >op acc, ; : DEC 1 >/n $48 >op acc, ;
\ ASM: PUSH/POP, XCHG/NOP
: stack, ( -- )
...
\ ASM: BYTE-SIZED WORDS, BITWISE LOGIC
...
\ ASSEMBLER EXAMPLE
\ fetch relative address of a word
: rel' ( "name" -- rel16 ) ' here @ - ;

\ "double-increment-print" defined using Forth
: dip ( n -- ) dup 1+ . ;

\ "double-increment-print" defined by inlining assembly
: dip ( n -- ) [
  [BP] AX MOV
    AX AX ADD
       AX INC
  AX [BP] MOV
   rel' . JMP  \ jump to "dot" subroutine
] ;
